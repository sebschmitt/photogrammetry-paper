...
Unter Structure from Motion, kurz SfM, versteht man eine Methode zur Rekonstruktion von 3D Punkte.
Im Gegensatz zur normalen Photogrammetrie, bei der zuvor bekannte 3D Punkte benötigt werden, wird bei SfM die Geometrie der Szene, Kamerapositionen und Orientierung automatisch berechnet.
...
Dieses Vorgehen bietet sich also vor allem für Mengen von Bildern an, welche sich zu einem hohen Grad überlappen aber gleichzeitig eine dreidimensionale Struktur vollständig zeigen.
...
Definiert ist diese als der Winkel zwischen zwei Graden, welche von verschiedenen Standorten auf denselben Punkt (ein Objekt) gerichtet sind (Siehe dazu \autoref{fig:theory-sfm-parallax}) \cite{stockmann_2001}.
...
Um die Kameraposition sowie die Geometrie der Szene simultan zu rekonstruieren werden automatisch Feature in den Bildern identifiziert. Diese Feature werden dann über mehrere Bilder verfolgt was eine erste Annahme zu den Kamerapositionen sowie den Objektkoordinaten erlaubt welche iterativ mit nicht-linearer least-squares minimisation verfeinert werden können. ~\cite[Kapitel 1.3]{westoby_2012}
